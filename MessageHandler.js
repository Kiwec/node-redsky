...
MessageHandler.prototype.clean = function(msg) {
  ...
  var noImg = noSmiley.replace(
    /<img.?src="(.*?)".*?>/g,
    this.skyChat.config.url + "/$1"
  );
...
MessageHandler.prototype.format = function(msg) {
  switch (msg.message_type) {
    ...
    case "user_me":
      return (
        "* " + this.formatColor(msg) + " " + colors.italic(this.clean(msg))
      );
    case "user_mp":
...
MessageHandler.prototype.formatColor = function(msg) {
  switch (msg.color) {
    case "#ef4848":
      return colors.red.bold(msg.pseudo);
    case "#457dbb":
      return colors.red(msg.pseudo);
    case "#bf00ff":
      return colors.green(msg.pseudo);
    case "#046380":
      return colors.blue(msg.pseudo);
    case "#85c630":
      return colors.cyan(msg.pseudo);
    case "#ffab46":
      return colors.yellow(msg.pseudo);
    case "#f5a6bf":
      return colors.magenta(msg.pseudo);
    default:
      return colors.white(msg.pseudo);
  }
};
...
MessageHandler.prototype.handleBotMessage = function(msg) {
  ...
  if (cleanMsg.indexOf(" a donné ") !== -1) {
    var giveMsg = cleanMsg.match(
      /(.+) a donné (\d+)\$ à (.+) \( commission (\d+)\$ \)/
    );
    ...
    var match = cleanMsg.match(
      /Nombre entre (\d+) et (\d+) demandé par (.+): (\d+)/
    );
    if (!match) {
